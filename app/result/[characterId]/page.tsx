...
export default function ResultPage() {
  ...
  const isAwaitingFeedback =
    chatState.isGameOver && !judgeFeedback && !feedbackError && (feedbackLoading || !feedback);

  if (isAwaitingFeedback) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#f4f7fb] px-4 py-8 text-slate-900">
        <div className="w-full max-w-md rounded-[2rem] bg-white p-8 text-center shadow-sm ring-1 ring-black/5">
          <div className="mx-auto h-12 w-12 animate-spin rounded-full border-4 border-slate-200 border-t-slate-900" />
          <h1 className="mt-6 text-xl font-bold text-slate-900">결과표 생성 중</h1>
          <p className="mt-2 text-sm leading-6 text-slate-500">
            저지 모델이 대화 전체를 채점하고 있어요. 보통 10~20초 정도 걸려요.
          </p>
          <p className="mt-6 text-xs text-slate-400">
            {character.name} · 최종 {character.scoreLabel ?? "성공 점수"} {chatState.affection}/100
          </p>
        </div>
      </main>
    );
